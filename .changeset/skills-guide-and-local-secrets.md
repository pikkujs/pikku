...
New `pikku-guide` skill: how `pikku scenario guide` compiles a user guide from the scenario suite — page markers, the capture step, `.guide.lock` staleness, `document: false`, and the traps that make a block come out empty or a run refused.
