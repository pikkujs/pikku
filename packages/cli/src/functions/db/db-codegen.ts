...
type Classification = 'public' | 'private' | 'secret' | 'encrypted'
...
function emitManifest(tables: TableSchema[], explicitAnnotations: AnnotationMap): string {
...
export async function generateSchemaTypes(
  ...
  try { existingSchema = readFileSync(options.outFile, 'utf8') } catch { /* ok */ }
  try { existingCoercion = readFileSync(options.coercionFile, 'utf8') } catch { /* ok */ }
  if (options.manifestFile) {
    try { existingManifest = readFileSync(options.manifestFile, 'utf8') } catch { /* ok */ }
  }
