...
const CanvasDrawerContext = createContext<CanvasDrawerContextType | undefined>(undefined);

export const useCanvasDrawerContext = () => {
  const context = useContext(CanvasDrawerContext);
  if (!context) {
    throw new Error("useCanvasDrawerContext must be used within CanvasDrawerProvider");
  }
  return context;
};
...
export const CanvasDrawerProvider: React.FunctionComponent<CanvasDrawerProviderProps> = ({
  children,
}) => {
