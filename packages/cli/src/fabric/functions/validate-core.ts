...
export async function runFabricValidate(
  ...
  // .pikku/ is generated by the CLI; committing it bloats PRs and causes stale-codegen issues
  ...
  try {
    ...
    const covered = lines.some(
      (l) =>
        l === '.pikku' || l === '.pikku/' || l === '/.pikku' || l === '/.pikku/'
    )
    if (!covered) {
      w(
        'pikku-dir-not-gitignored',
        '.pikku/ is not listed in .gitignore — generated codegen artifacts will pollute commits and PRs',
        gitignorePath,
        'Add ".pikku" to .gitignore'
      )
    }
  } catch {
    w(
      'gitignore-missing',
      '.gitignore not found at project root — generated .pikku/ artifacts may be committed accidentally',
      gitignorePath,
      'Create .gitignore and add ".pikku" to it'
    )
  }
