...
test('parseDbEnums reads the union members of each exported type', () => {
  ...
  assert.deepEqual(byName.ParticipantDietaryTag, ['omnivore', 'vegetarian', 'vegan'])
...
test('an exact value-set match types the group against the DB enum', () => {
  const keys = ['enum__dietary__omnivore', 'enum__dietary__vegetarian', 'enum__dietary__vegan']
...
test('a non-DB group (no overlap) stays catalog-typed and warns nothing', () => {
  ...
  const keys = ['enum__email_kind__send_welcome', 'enum__email_kind__send_receipt']
  const src = generateEnumsSource(keys, {
    dbEnums: parseDbEnums(ENUMS_GEN),
    enumsImport: '#pikku/db/enums.gen',
    onWarn: (m) => warnings.push(m),
  })
  assert.match(src, /export const emailKind = \{[\s\S]*?\} satisfies EnumLabel<"send_welcome" \| "send_receipt">/)
  assert.equal(warnings.some((w) => w.includes('email_kind')), false)
})
...
test('a group with an extra member warns about catalog/DB drift', () => {
  ...
  const drift = warnings.find((w) => w.includes("'booking_status'") && w.includes('BookingStatus'))
...
test("an unmatched DB enum emits a label map referencing default keys ('emit')", () => {
  ...
  assert.match(src, /export const auditLogAction = \{[\s\S]*?\} satisfies EnumLabel<AuditLogAction>/)
  assert.match(src, /create: m\.enum__audit_log_action__create,/)
  assert.match(src, /import type \{ AuditLogAction, BookingStatus, ParticipantDietaryTag \} from/)
  assert.ok(warnings.some((w) => w.includes('AuditLogAction') && w.includes('enum__audit_log_action__create')))
})
