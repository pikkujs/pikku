...
const getBorderStyle = (position: BorderPosition, theme: any, borderColor?: string) => {
...
const getHighlightIconColor = (highlightType: HighlightType, theme: any): string | null => {
...
export const FlowNode: React.FunctionComponent<FlowNodeProps> = ({
  ...
  const runBgColor = React.useMemo(() => {
    ...
    if (!status && !hasInput && runContext.stepStates.size === 0 && runContext.runData?.status === "failed") {
      status = "failed";
    }
  ...
  }, [runContext?.selectedRunId, runContext?.stepStates, runContext?.runData?.status, nodeId, hasInput, theme]);

  const iconColor = highlightIconColor || (runBgColor ? "white" : (theme.colors.gray[5] || colorKey));
  ...
  return (
    <Box style={{ width: size, overflow: "visible" }}>
      ...
      {(label || subtitle) && (
        ...
        >
          {label && (
            <Text size="sm" fw={600} c={highlightIconColor || (labelDimmed ? "dimmed" : undefined)}>
              {label}
            </Text>
          )}
          {subtitle && (
