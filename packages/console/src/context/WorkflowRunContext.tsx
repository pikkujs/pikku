...
import { useWorkflowRun, useWorkflowRunSteps, useWorkflowVersion } from "@/hooks/useWorkflowRuns";
...
const WorkflowRunContext = createContext<WorkflowRunContextType | undefined>(undefined);
...
const buildStepNameToNodeIdMap = (workflowNodes: Record<string, any> | undefined): Map<string, string> => {
...
export const WorkflowRunProvider: React.FunctionComponent<WorkflowRunProviderProps> = ({
  children,
  workflowName,
  currentGraphHash,
  workflowNodes,
  ...
  const [selectedRunId, setSelectedRunIdState] = useState<string | null>(initialRunId);
  const [isCreatingRun, setIsCreatingRun] = useState(false);

  const { data: queryRunData, isLoading: runLoading } = useWorkflowRun(selectedRunId);
  const { data: querySteps, isLoading: stepsLoading } = useWorkflowRunSteps(selectedRunId);
  ...
  const stepStates = useMemo(() => {
    ...
    if (steps && Array.isArray(steps)) {
      for (const step of steps) {
        const nodeId = stepNameToNodeId.get(step.stepName)
  ...
  const { data: historicalVersion, isLoading: historicalLoading } = useWorkflowVersion(
