...
- 3cd4e13: `holdMilestoneLifecycle` now restores a milestone's `statusAt` when a note-rewriting
...
- 11ce9c0: Let the caller own the plan's closing instruction, and export `basePlan`.
...
- af6a29c: `knowledge validate` now checks plans, not just notes.
