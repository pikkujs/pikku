...
export class PikkuRPC {
  ...
  invoke = ((rpcName: string, data?: unknown) => {
    return this.pikkuFetch.post(`/rpc/${String(rpcName)}` as never, {
  ...
  startWorkflow: TypedStartWorkflow = async (workflowName, input) => {
    return (await this.pikkuFetch.post(
      `/workflow/${String(workflowName)}/start` as never,
      { data: input }
    )) as any
  }
  ...
  runWorkflow: TypedRunWorkflow = async (workflowName, input) => {
    return (await this.pikkuFetch.post(
      `/workflow/${String(workflowName)}/run` as never,
      { data: input }
    )) as any
  }
  ...
  workflowStatus: TypedWorkflowStatus = async (workflowName, runId) => {
    return (await this.pikkuFetch.get(
      `/workflow/${String(workflowName)}/status/${runId}` as never
  ...
  agent = {
    ...
    run: (async (agentName, input) => {
      return (await this.pikkuFetch.post(
        `/rpc/agent/${String(agentName)}` as never,
        input
      )) as any
    }) as TypedAgentRun,
    /**
    ...
    stream: async (agentName: string, input: Record<string, unknown>) => {
      return (await this.pikkuFetch.post(
        `/rpc/agent/${String(agentName)}/stream` as never,
        input
      )) as any
    },
    /**
    ...
    approve: async (agentName: string, input: Record<string, unknown>) => {
      return (await this.pikkuFetch.post(
        `/rpc/agent/${String(agentName)}/approve` as never,
        input
      )) as any
    },
  }
