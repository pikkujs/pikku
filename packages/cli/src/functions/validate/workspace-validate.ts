  ...
  if (!pikkuConfig) {
    ...
    if (!scaffold?.console) {
      e(
        ...
        pikkuConfigPath,
        'Add "console": true under "scaffold" in pikku.config.json — the console requires an authenticated session, so Better Auth must be set up (see the pikku-better-auth skill)'
      )
    }
  }
