...
export function makeDeterministicAnonName(
  ...
  if (ts.isPropertyAssignment(node.parent) && node === node.parent.initializer && ts.isIdentifier(node)) {
    const sym = checker.getSymbolAtLocation(node);
    if (sym) {
    ...
    for (const decl of decls) {
      ...
      if (ts.isShorthandPropertyAssignment(decl)) {
        ...
        if (shorthandSym && shorthandSym.declarations && shorthandSym.declarations.length > 0) {
          ...
          if (ts.isVariableDeclaration(shorthandDecl) && ts.isIdentifier(shorthandDecl.name)) {
            node = shorthandDecl.name;
            didResolve = true;
            break;
          } else if (ts.isFunctionDeclaration(shorthandDecl) && shorthandDecl.name && ts.isIdentifier(shorthandDecl.name)) {
            node = shorthandDecl.name;
            didResolve = true;
            break;
          } else if (ts.isParameter(shorthandDecl) && ts.isIdentifier(shorthandDecl.name)) {
            node = shorthandDecl.name;
            didResolve = true;
            break;
          } else if (ts.isPropertyDeclaration(shorthandDecl) && ts.isIdentifier(shorthandDecl.name)) {
            node = shorthandDecl.name;
            didResolve = true;
            break;
          } else if (ts.isMethodDeclaration(shorthandDecl) && ts.isIdentifier(shorthandDecl.name)) {
            node = shorthandDecl.name;
            didResolve = true;
            break;
          }
 ...
 */
export function extractFunctionName(callExpr: ts.Node, checker: ts.TypeChecker): ExtractedFunctionName {
  ...
  if (ts.isIdentifier(callExpr) && callExpr.parent && ts.isPropertyAssignment(callExpr.parent)) {
        ...
        if (ts.isVariableDeclaration(decl) && decl.initializer) {
          if (ts.isCallExpression(decl.initializer) && 
            ...
            if (firstArg && (ts.isArrowFunction(firstArg) || ts.isFunctionExpression(firstArg))) {
              ...
              if (ts.isVariableDeclaration(decl) && isNamedExport(decl) && ts.isIdentifier(decl.name)) {
  ...
  if (ts.isCallExpression(callExpr)) {
    ...
    if (args.length > 0) {
      ...
      if (firstArg && ts.isObjectLiteralExpression(firstArg)) {
        ...
        for (const prop of firstArg.properties) {
          if (ts.isPropertyAssignment(prop) && ts.isIdentifier(prop.name) && prop.name.text === 'func') {
            if (ts.isIdentifier(prop.initializer)) {
              ...
              if (funcSym) {
                ...
                if (resolvedFuncSym.flags & ts.SymbolFlags.Alias) {
                  resolvedFuncSym = checker.getAliasedSymbol(resolvedFuncSym) ?? resolvedFuncSym;
                }
                ...
                if (funcDecls.length > 0) {
                  ...
                  if (ts.isVariableDeclaration(funcDecl) && funcDecl.initializer) {
                    if (ts.isCallExpression(funcDecl.initializer) && 
                      ...
                      if (firstArg && (ts.isArrowFunction(firstArg) || ts.isFunctionExpression(firstArg))) {
                        ...
                        // Check if the variable is exported
                        if (isNamedExport(funcDecl) && ts.isIdentifier(funcDecl.name)) {
                    ...
                    } else if (ts.isFunctionExpression(funcDecl.initializer) || ts.isArrowFunction(funcDecl.initializer)) {
                    ...
                    if (funcDecl.modifiers?.some(m => m.kind === ts.SyntaxKind.ExportKeyword) && 
                        funcDecl.name && ts.isIdentifier(funcDecl.name)) {
                      result.exportedName = funcDecl.name.text;
                    } else if (funcDecl.name && ts.isIdentifier(funcDecl.name)) {
          ...
          } else if (ts.isShorthandPropertyAssignment(prop) && ts.isIdentifier(prop.name) && prop.name.text === 'func') {
            ...
            if (shorthandSym && shorthandSym.declarations && shorthandSym.declarations.length > 0) {
              ...
              if (ts.isVariableDeclaration(shorthandDecl) && shorthandDecl.initializer) {
                if (ts.isCallExpression(shorthandDecl.initializer) && 
                  ...
                  if (firstArg && (ts.isArrowFunction(firstArg) || ts.isFunctionExpression(firstArg))) {
                    ...
                    // Check if the variable is exported
                    if (isNamedExport(shorthandDecl) && ts.isIdentifier(shorthandDecl.name)) {
                ...
                } else if (ts.isFunctionExpression(shorthandDecl.initializer) || ts.isArrowFunction(shorthandDecl.initializer)) {
                ...
                if (shorthandDecl.modifiers?.some(m => m.kind === ts.SyntaxKind.ExportKeyword) && 
                    shorthandDecl.name && ts.isIdentifier(shorthandDecl.name)) {
                  result.exportedName = shorthandDecl.name.text;
                } else if (shorthandDecl.name && ts.isIdentifier(shorthandDecl.name)) {
                  // If not exported, still capture the function name
                  result.functionName = shorthandDecl.name.text;
                }
    ...
    if (sym) {
      ...
      if (decls.length > 0) {
        ...
        if (ts.isVariableDeclaration(decl) && decl.initializer) {
          ...
          } else if (ts.isFunctionExpression(decl.initializer) || ts.isArrowFunction(decl.initializer)) {
  ...
  else if (ts.isShorthandPropertyAssignment(parent) && ts.isIdentifier(parent.name)) {
    result.propertyName = parent.name.text;
  }
