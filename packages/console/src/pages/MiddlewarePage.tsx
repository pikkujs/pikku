...
const MiddlewareTable: React.FunctionComponent<{ items: MiddlewareItem[]; loading?: boolean }> = ({ items, loading }) => {
...
export const MiddlewarePage: React.FunctionComponent = () => {
    ...
    for (const [, inst] of Object.entries(instances) as [string, any][]) {
      ...
      if (defId && !seen.has(defId)) {
        ...
        result.push({
          id: `middleware::def::${defId}`,
          name: defId,
          data: { _id: defId, name: defId, sourceFile: inst.sourceFile, isFactoryCall: inst.isFactoryCall },
        });
      }
    }
  ...
  return (
    <PanelProvider>
      <ResizablePanelLayout header={<DetailPageHeader icon={Layers} category="Middleware" docsHref="https://pikkujs.com/docs/middleware" />} showTabs={false} hidePanel={!loading && items.length === 0} emptyPanelMessage="Select a middleware to view its details">
