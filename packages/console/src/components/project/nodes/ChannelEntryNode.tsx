...
const HandlerRow: React.FunctionComponent<HandlerRowProps> = ({ label, handleId }) => {
  return (
    <Box
      px="xs"
      pl="md"
...
export const ChannelEntryNode: React.FunctionComponent<NodeProps<ChannelEntryNodeData>> = ({
  data,
}) => {
  ...
  return (
    ...
    >
      ...
      <Stack gap={0} py="xs">
        <Box px="md" pb={4}>
          <Text size="md" fw={600}>{data.channelName}</Text>
          <Text size="sm" c="dimmed">{data.route}</Text>
        </Box>
