...
const SESSION_WIRES = new Set(["session", "setSession", "clearSession", "getSession", "hasSessionChanged"]);
...
const WiredToSection: React.FunctionComponent<{ functionName: string }> = ({ functionName }) => {
      ...
      <Table verticalSpacing={4} horizontalSpacing="xs">
        ...
        <Table.Tbody>
          {allWirings.map((w) => (
            <Table.Tr key={w.id}>
              <Table.Td>
                <Anchor component={Link} to={TYPE_HREF[w.type] || "#"} underline="never" size="sm" ff="monospace">
                  {w.name}
                </Anchor>
              </Table.Td>
              <Table.Td>
      ...
      {description !== undefined && (
        <Box>
          ...
          {description ? (
            ...
            <Text size="md" c="dimmed">No description</Text>
          )}
        </Box>
      )}
      ...
      <Box>
        ...
        {wires && wires.wires.length > 0 ? (
          <Group gap={6}>
            ...
            {wires.wires.filter((w) => !SESSION_WIRES.has(w)).map((w: string) => (
