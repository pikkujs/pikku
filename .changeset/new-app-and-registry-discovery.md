...
**`pikku new app <slug> --serves <group> --personas <ids>`** creates a second
frontend by cloning the one that already works. It skips `node_modules` and
`src/paraglide` (compiled from `messages/` on first run, so copying it forward
ships one app's strings inside another), re-points the clone's `package.json`
at its own name, dev/preview port and `--tsBuildInfoFile`, stamps
