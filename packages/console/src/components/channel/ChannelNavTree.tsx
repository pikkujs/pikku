...
export const ChannelNavTree: React.FunctionComponent<ChannelNavTreeProps> = ({
  ...
  const hasResults = filteredHandlers.length > 0 || filteredCategories.length > 0;
        ...
        {filteredHandlers.map((h) => {
          ...
          return (
            <NavLink
              key={h}
              label={
                <Text size="sm" fw={isHandlerActive(h) ? 600 : 400} c={exists ? undefined : "dimmed"}>
                  {h}
                </Text>
              }
              ...
              >
                {Object.keys(actions).map((action) => (
                  <NavLink
                    key={action}
                    label={
                      <Text size="sm" fw={isActionActive(category, action) ? 600 : 400}>
                        {action}
                      </Text>
                    }
