...
/**
 * `pikkuAgentScorer` grades in code and `pikkuAgentJudge` grades with a model, which
...
const LANES: Record<string, 'fast' | 'slow'> = {
  pikkuAgentScorer: 'fast',
  pikkuAgentJudge: 'slow',
}

export const addAgentScorer: AddWiring = (logger, node, checker, state) => {
