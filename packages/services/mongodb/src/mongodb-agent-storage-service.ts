...
export class MongoDBAgentStorageService
  implements AgentStorageService, AgentRunStateService
{
  private initialized = false
  private threads!: Collection<AgentThreadDoc>
  private agentMessages!: Collection<AgentMessageDoc>
  private toolCalls!: Collection<AgentToolCallDoc>
  private workingMemory!: Collection<AgentWorkingMemoryDoc>
  private agentRuns!: Collection<AgentRunDoc>
  private agentRunScores!: Collection<AgentRunScoreDoc>
  ...
  public async init(): Promise<void> {
    ...
    this.threads = this.db.collection<AgentThreadDoc>('agent_threads')
    this.agentMessages = this.db.collection<AgentMessageDoc>('agent_message')
    this.toolCalls = this.db.collection<AgentToolCallDoc>('agent_tool_call')
    this.workingMemory = this.db.collection<AgentWorkingMemoryDoc>(
      'agent_working_memory'
    )
    this.agentRuns = this.db.collection<AgentRunDoc>('agent_run')
    this.agentRunScores =
      this.db.collection<AgentRunScoreDoc>('agent_run_score')

    await this.threads.createIndex({ resourceId: 1 })

    await this.agentMessages.createIndex({ threadId: 1, createdAt: 1 })
    ...
    await this.agentRuns.createIndex({ threadId: 1, createdAt: 1 })

    await this.agentRunScores.createIndex({ runId: 1, createdAt: 1 })
  ...
  async deleteThread(threadId: string): Promise<void> {
    ...
    await this.agentMessages.deleteMany({ threadId })
    await this.agentRuns.deleteMany({ threadId })
  ...
  async getMessages(
    ...
    if (options?.cursor) {
      const cursorRow = await this.agentMessages.findOne({
        _id: options.cursor,
      })
      if (cursorRow) {
    ...
    if (options?.cursor || options?.lastN) {
      const innerResult = await this.agentMessages
        .find(filter)
        .sort({ createdAt: -1 })
      ...
      msgResult = await this.agentMessages
        .find(filter)
        .sort({ createdAt: 1 })
        .toArray()
    }
    ...
    for (const row of msgResult) {
      let parsedContent: AgentMessage['content'] = row.content ?? undefined
  ...
  async saveMessages(
    ...
    if (nonToolMessages.length > 0) {
      await this.agentMessages.insertMany(
  ...
  async createRun(run: CreateRunInput): Promise<string> {
    ...
    await this.agentRuns.insertOne({
      _id: runId,
  ...
  async updateRun(
    ...
    await this.agentRuns.updateOne({ _id: runId }, { $set: setValues })
  ...
  async getRun(runId: string): Promise<AgentRunState | null> {
    const row = await this.agentRuns.findOne({ _id: runId })
    if (!row) return null
  ...
  async getRunsByThread(threadId: string): Promise<AgentRunState[]> {
    const result = await this.agentRuns
      .find({ threadId })
      .sort({ createdAt: -1 })
      .toArray()
  ...
  async saveScore(score: SaveScoreInput): Promise<void> {
    await this.agentRunScores.insertOne({
  ...
  async getScores(runId: string): Promise<AgentRunScore[]> {
    const rows = await this.agentRunScores
      .find({ runId })
      .sort({ createdAt: 1 })
      .toArray()
    return rows.map((row) => ({
