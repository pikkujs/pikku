...
export const ShellHeader = <T extends string = string>({
  ...
  const candWidth = (c: Candidate): number => {
    ...
    const showFunnel =
      hidden.length > 0 || (!!search && !c.searchInline) || selInDrawer
    const parts: number[] = []
    ...
    return (
      used.reduce((s, p) => s + p, 0) +
      GAP * Math.max(0, used.length - 1) +
      extra +
      SAFETY
    )
  }
  ...
  if (w && width > 0) {
    chosen =
      candidates.find((c) => candWidth(c) <= width) ??
  ...
  const renderSearchField = (measurement: boolean) =>
    search ? (
      <TextInput
        ...
        styles={{
          root: { flexShrink: 0 },
          input: { height: CONTROL_H, minHeight: CONTROL_H },
        }}
      />
    ) : null
  ...
  return (
    <>
      ...
      >
        ...
        {selection &&
          measureNode('selCycle', <CycleSwitch selection={selection} />)}
        {filters.map((f) =>
          measureNode(
            'filter:' + f.key,
