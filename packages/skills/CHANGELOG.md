...
- 828b5b5: `pikku-knowledge` now says that a milestone which has reached `built` is closed: neither its note nor its plan is edited again. The rule was implied by everything around it and written down nowhere, so an agent tidying a finished milestone was doing something the skill never told it not to.
...
- 96b65ae: Document how an addon ships database tables: `pikku db export`, the artifact the
  consumer resolves through the package name, and the silent failure when it is
  neither exported nor packed.
- c4ee98d: **pikku-build** gains `references/design.md`: the composition doctrine that no theme can supply. An app built to the letter of the skill still came out looking like nobody had decided anything — a form at the top of every list screen, sixty identical cards each carrying two filled buttons, an "urgent" group holding half the fridge, a scaffolded login page with a Google button the app had no credentials for, and the whole thing judged against a dev database full of `Ripe peaches d193e2aa` left behind by scenario runs. The new reference names the bar (six questions a screen has to answer), the composition rules that get it there, the screens agents forget — login, navigation, empty states — and the reset-and-restart that has to happen before any of it is judged. §8 now says out loud that half of it belongs at §6, because a design pass over eight finished milestones is a repaint.
...
- 28f39f2: Add the `pikku-blueprint-to-fabric` and `pikku-admin-to-fabric` skills, which rebuild a legacy app (and its generated back-office admin) as a Fabric app from a `.knowledge/` Product Blueprint.
...
- 5ace170: Fix the `#pikku` import specifiers the skills teach.
...
- 8519a73: Say plainly that the dev seed is local only. No deploy applies it, so the test for a row is not whether it looks realistic but whether the app would be broken without it in production — a venue, a catalogue, a tenant is configuration and belongs in a migration. Names the failure it causes: a stage whose pages return 200 while its first data read throws.

## 0.12.15

### Patch Changes

- 56d6fde: Tag the skills Fabric sandboxes rely on with `installGroups: [fabric]`, so `pikku skills install --fabric` pulls them
- 56d6fde: `pikku-info` becomes `pikku-meta`, and now owns the whole metadata surface rather
