...
const OverviewView: React.FunctionComponent<{ channelName: string; channel: ChannelMeta }> = ({
  channelName,
  channel,
}) => (
  <Stack gap="sm">
    <Text size="lg" fw={600}>{channelName}</Text>
...
const HandlerView: React.FunctionComponent<{ handler: string; channel: ChannelMeta }> = ({
  handler,
  channel,
}) => {
  ...
  return (
    <Stack gap="sm">
      <Text size="lg" fw={600}>{handler}</Text>
      <Divider />
...
export const ChannelDetailView: React.FunctionComponent<ChannelDetailViewProps> = ({
  ...
  return (
    <Stack gap="md" p="md" style={{ height: "100%", overflow: "auto" }}>
      {!selected && <OverviewView channelName={channelName} channel={channel} />}
