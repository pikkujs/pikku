...
import { ChannelNavTree, type ChannelSelection } from "@/components/channel/ChannelNavTree";
import { ChannelDetailView } from "@/components/channel/ChannelDetailView";
import type { ChannelMeta } from "@pikku/core/channel";

const getSelectedFuncId = (channel: ChannelMeta, selected: ChannelSelection): string | null => {
  if (!selected) return null;
  if (selected.type === "handler") {
    return channel[selected.handler as "connect" | "disconnect" | "message"]?.pikkuFuncId ?? null;
  }
  return channel.messageWirings?.[selected.category]?.[selected.action]?.pikkuFuncId ?? null;
};
...
export const ChannelPageClient: React.FunctionComponent = () => {
  ...
  const channelItems = useMemo(() => {
    ...
    return Object.entries(meta.channelsMeta).map(([name, ch]: [string, any]) => ({
      name,
