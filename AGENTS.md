...
- `outDir/db/schema.d.ts` — Kysely types with `Private<T>`/`Secret<T>`/`Encrypted<T>` brands; regenerated by `pikku db migrate`. Schema changes go through SQL migrations first.
- `outDir/db/classification.gen.ts` — classification manifest (per-column `private`/`secret`/`encrypted`/`public` + anonymize strategy); also regenerated by `pikku db migrate`. To change a column's classification, update the `-- @private`, `-- @secret`, `-- @encrypted`, or `-- @public` SQL comment annotation and re-run migrate.
