...
function defineTestSuite(name: string, getDb: () => Db) {
  describe(`MongoDBChannelStore [${name}]`, () => {
    let store: MongoDBChannelStore

    before(async () => {
      store = new MongoDBChannelStore(getDb())
      await store.init()
    })
  ...
  describe(`MongoDBEventHubStore [${name}]`, () => {
    let store: MongoDBEventHubStore

    before(async () => {
      store = new MongoDBEventHubStore(getDb())
      await store.init()
    })
  ...
  describe(`MongoDBWorkflowService [${name}]`, () => {
    let service: MongoDBWorkflowService
    let runService: MongoDBWorkflowRunService

    before(async () => {
      service = new MongoDBWorkflowService(getDb())
      runService = new MongoDBWorkflowRunService(getDb())
      await service.init()
    })
  ...
  describe(`MongoDBWorkflowRunService [${name}]`, () => {
    let runService: MongoDBWorkflowRunService
  ...
  describe(`MongoDBAIStorageService [${name}]`, () => {
    let storage: MongoDBAIStorageService

    before(async () => {
      storage = new MongoDBAIStorageService(getDb())
      await storage.init()
    })
  ...
  describe(`MongoDBDeploymentService [${name}]`, () => {
    let service: MongoDBDeploymentService

    before(async () => {
      service = new MongoDBDeploymentService(
  ...
  describe(`MongoDBSecretService [${name}]`, () => {
    ...
    test('rotateKEK re-wraps all secrets', async () => {
      ...
      const oldService = new MongoDBSecretService(getDb(), { key: kek })
      await oldService.init()
      await oldService.setSecretJSON('rotate-test', { important: 'data' })

      const rotatedService = new MongoDBSecretService(getDb(), {
        key: newKEK,
        keyVersion: 2,
        previousKey: kek,
      })
      ...
      const newOnlyService = new MongoDBSecretService(getDb(), {
        key: newKEK,
        keyVersion: 2,
      })
  ...
  describe(`MongoDBAgentRunService [${name}]`, () => {
    let agentService: MongoDBAgentRunService

    before(async () => {
      agentService = new MongoDBAgentRunService(getDb())
    })
...
describe('MongoDB Services - In-Memory', () => {
  ...
  defineTestSuite('MongoMemory', () => db)
})
...
describe(
  ...
  () => {
    ...
    defineTestSuite('Real MongoDB', () => db)
  }
)
