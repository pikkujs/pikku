...
describe('serializeWorkflowTypes', () => {
  ...
  // A project that only ships workflows should not carry the scenario surface:
  // scenarios are a testing primitive and get their own barrel.
  test('the scenario surface is not in the workflow barrel', () => {
    const result = emit()
    for (const name of [
      'pikkuScenario',
      'pikkuScenarioStep',
      ...
      assert.doesNotMatch(
        result,
        new RegExp(`\\b${name}\\b`),
        `expected '${name}' to live in the scenario barrel`
      )
    }
  })

  test('TypedWorkflow stays exported for the scenario barrel to extend', () => {
    assert.match(
      emit(),
