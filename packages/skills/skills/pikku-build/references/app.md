...
like the template: **direction** (deciding what it should look like — no
open-source tool does this for you) and **critique** (judging how well the built
screens execute it — `impeccable` does this well, and it is free). Impeccable
will happily award a clean bill of health to a perfectly-executed default, so
skipping direction ships Neutral with good spacing.

**Read [design.md](references/design.md)** for both halves: authoring the theme in
`packages/mantine-theme` (and where the direction gets written down), which
Mantine components to compose with instead of tables and text, and how to run
impeccable — including the two things that decide whether its findings are worth
anything, namely feeding it screenshots rather than source, and critiquing at a
phone width as well as desktop.
...
- [design.md](references/design.md) — theme direction and critique (§8), and
  [theming.md](references/theming.md) for the theme file format itself
- [ship.md](references/ship.md) — deploying, and the Fabric-readiness contract (§9)
