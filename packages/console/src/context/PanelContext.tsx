...
export type PanelType = "function" | "http" | "channel" | "rpc" | "scheduler" | "queue" | "cli" | "mcp" | "workflowStep" | "workflow" | "trigger" | "triggerSource" | "middleware" | "permission" | "agent" | "secret" | "variable";
...
interface PanelContextType {
  ...
  navigateInPanel: (type: PanelType, id: string, title: string, metadata?: any) => void;
  goBack: () => void;
...
export const PanelProvider: React.FunctionComponent<PanelProviderProps> = ({
  ...
  const openFunction = useCallback(
    (functionName: string, metadata?: any) => {
      openPanelGeneric("function", functionName, metadata?.summary || functionName, metadata);
    },
    [openPanelGeneric]
  );
  ...
  const closePanel = useCallback((id: string) => {
    ...
    setActivePanelState((current) => {
      if (current === id) {
        const remaining = Array.from(panels.keys()).filter((key) => key !== id);
