...
export interface SerializableInspectorState {
  ...
  scorers: {
    scorersMeta: InspectorState['scorers']['scorersMeta']
    files: [string, { path: string; exportedName: string }][]
  }
  cli: {
  ...
  agentMiddleware: {
    definitions: InspectorState['agentMiddleware']['definitions']
  }
  permissions: {
...
export function serializeInspectorState(
  ...
  return {
    ...
    scorers: {
      ...
      files: Array.from(state.scorers?.files?.entries() ?? []),
    },
    cli: {
      meta: state.cli.meta,
    ...
    agentMiddleware: {
      definitions: state.agentMiddleware.definitions,
    },
    permissions: {
...
export function deserializeInspectorState(
  ...
  return {
    ...
    scorers: {
      ...
      files: new Map(data.scorers?.files || []),
    },
    cli: {
      meta: data.cli.meta,
    ...
    agentMiddleware: {
      definitions: data.agentMiddleware?.definitions || {},
    },
    permissions: {
