...
const PermissionsTable: React.FunctionComponent<{ items: PermissionItem[]; loading?: boolean }> = ({ items, loading }) => {
  ...
  const columns = useMemo(() => [
    {
      ...
      render: (item: PermissionItem) => (
        <>
          ...
          {item.data?.description && <Text size="xs" c="dimmed" lineClamp={1}>{item.data.description}</Text>}
        </>
      ),
    },
    {
      ...
      render: (item: PermissionItem) => {
        ...
        const sessionWires = new Set(["session", "setSession", "clearSession", "getSession", "hasSessionChanged"]);
        ...
        return (
          <Group gap={4} wrap="nowrap">
            ...
            {nonSessionWires.map((w) => <PikkuBadge key={w} type="dynamic" badge="wire" value={w} />)}
            {item.data?.factory && <PikkuBadge type="flag" flag="factory" />}
            {item.data?.exportedName === null && <PikkuBadge type="flag" flag="local" />}
          </Group>
        );
      },
    },
...
export const PermissionsPage: React.FunctionComponent = () => {
  ...
  const items = useMemo((): PermissionItem[] => {
    ...
    for (const group of [...Object.values(httpGroups), ...Object.values(tagGroups)] as any[]) {
      for (const instanceId of group.instanceIds || []) {
        if (!seen.has(instanceId)) {
          ...
          result.push({
            ...
            name: instanceId,
            data: { _id: instanceId, name: instanceId, sourceFile: group.sourceFile },
          });
        }
      }
    }
  ...
  return (
    <PanelProvider>
      <ResizablePanelLayout header={<DetailPageHeader icon={Shield} category="Permissions" docsHref="https://pikkujs.com/docs/permissions" />} showTabs={false} hidePanel={!loading && items.length === 0} emptyPanelMessage="Select a permission to view its details">
