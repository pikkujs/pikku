...
describe('runKnowledgeValidate on a bundle written before the rename', () => {
  const LEGACY = {
    'knowledge/index.md': '---\ntype: overview\n---\nThe app.',
    'knowledge/slices/index.md': '---\ntype: overview\n---\nBuildable pieces.',
    'knowledge/slices/01-a.md': MILESTONE.replace(
      'type: milestone',
      'type: slice'
    ),
  }

  test('slices/ holding type: slice is a clean bundle, not an unknown section', async () => {
    const result = await validate(await project(LEGACY))
    assert.deepEqual(result.findings, [])
    assert.equal(result.ok, true)
  })

  test('a legacy milestone is still held to the milestone rules', async () => {
    // The rename cannot be a way to stop being checked: a note that was a slice
    // yesterday is a milestone today, and dropping it from the type test would
    // silently retire every gate on every bundle already written.
    const result = await validate(
      await project({
        ...LEGACY,
        'knowledge/slices/01-a.md': MILESTONE.replace(
