...
import { getBranchNodeConfig } from '../components/nodes/BranchNode'
import { getFunctionNodeConfig } from '../components/nodes/FunctionNode'
import { getSleepNodeConfig } from '../components/nodes/SleepNode'
import { getSuspendNodeConfig } from '../components/nodes/SuspendNode'
import { getInlineNodeConfig } from '../components/nodes/InlineNode'
import { getGenericNodeConfig } from '../components/nodes/GenericNode'
import { getFanoutNodeConfig } from '../components/nodes/FanoutNode'
import { getReturnNodeConfig } from '../components/nodes/ReturnNode'
import { getCancelNodeConfig } from '../components/nodes/CancelNode'
import { getSwitchNodeConfig } from '../components/nodes/SwitchNode'
import { getArrayPredicateNodeConfig } from '../components/nodes/ArrayPredicateNode'
import { getFilterNodeConfig } from '../components/nodes/FilterNode'
import { getParallelNodeConfig } from '../components/nodes/ParallelNode'
import { getSetNodeConfig } from '../components/nodes/SetNode'
import {
...
} from '../components/nodes/WiringNode'
import { getChannelWiringNodeConfig } from '../components/nodes/ChannelWiringNode'
...
/** A branch's condition is the readable thing about the edge, so it rides the
 *  edge as a label rather than costing the graph a node of its own. */
function branchLabel(condition: any, fallback: string): string {
...
function processStep(
  ...
  switch (stepType) {
    ...
    case 'suspend': {
      const suspendNode = getSuspendNodeConfig(stepNodeId, position, step)
      nodes.push(suspendNode)
      break
    }
    case 'inline': {
      const functionNode = getFunctionNodeConfig(
      ...
      if (step.thenEntry) {
        edges.push(
          createEdge(
            ...
            nodeId,
            step.thenEntry,
            branchLabel(step.condition, 'then'),
          ...
          if (branch.entry) {
            edges.push(
              createEdge(
                ...
                nodeId,
                branch.entry,
                branchLabel(branch.condition, `branch ${branchIndex + 1}`),
