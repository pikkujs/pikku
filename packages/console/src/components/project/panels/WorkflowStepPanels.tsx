import { Stack, Text, Box, Code, Group, Table, Loader, Card } from "@mantine/core";
...
type InputBadgeType = "$ref" | "$trigger" | "$state" | "$template" | "$static" | "$expression";
...
const parseInputValue = (value: any): ParsedInputValue => {
  if (value === null || value === undefined) {
    return { type: "$static", displayValue: <Text c="dimmed" size="sm">null</Text> };
  }
  ...
  if (typeof value === "object" && value.$template) {
    ...
    for (let i = 0; i < template.parts.length; i++) {
      ...
      if (i < template.expressions.length) {
        ...
        if (typeof expr === "object" && expr.$ref) {
          const badgeType: InputBadgeType = expr.$ref === "trigger" ? "$trigger" : "$ref";
          const displayPath = expr.path ? `.${expr.path}` : "";
          elements.push(
            <PikkuBadge key={`expr-${i}`} type="label" color={workflowInputTypeDefs[badgeType]?.color || "gray"}>
              {expr.$ref + displayPath}
            </PikkuBadge>
          );
        } else {
          elements.push(
            <PikkuBadge key={`expr-${i}`} type="label" color={workflowInputTypeDefs.$expression?.color || "cyan"}>
              {String(expr)}
            </PikkuBadge>
          );
        }
    ...
    return {
      type: "$template",
      displayValue: <Group gap={4} wrap="wrap">{elements}</Group>,
    };
  }

  if (typeof value === "object" && value.$ref) {
    const isSpecial = value.$ref === "trigger" || value.$ref === "$item";
    const badgeType: InputBadgeType = value.$ref === "trigger" ? "$trigger" : "$ref";
    const displayPath = value.path ? `.${value.path}` : "";

    return {
      type: badgeType,
      displayValue: <Text size="sm" ff="monospace">{value.$ref + displayPath}</Text>,
  ...
  if (typeof value === "object" && value.$state) {
    return {
      type: "$state",
      displayValue: <Text size="sm" ff="monospace">{value.$state}</Text>,
    };
  }
  ...
  if (typeof value === "object" && value.$expression) {
    return {
      type: "$expression",
      displayValue: <Text size="sm" ff="monospace">{value.$expression}</Text>,
    };
  }
  ...
  return {
    type: "$static",
    displayValue: <Text size="sm" ff="monospace">{String(value)}</Text>,
  };
};

export const WorkflowStepConfiguration: React.FunctionComponent<WorkflowStepPanelProps> = ({
  stepId,
}) => {
...
export const WorkflowStepInput: React.FunctionComponent<WorkflowStepPanelProps> = ({
  stepId,
}) => {
  const node = useWorkflowNode(stepId);
        ...
        {hasInput ? (
          <Card.Section>
            <Table verticalSpacing={4} horizontalSpacing="xs">
              <Table.Thead>
                <Table.Tr>
                  <Table.Th c="dimmed" fw={500} fz="xs">Parameter</Table.Th>
                  <Table.Th c="dimmed" fw={500} fz="xs">Type</Table.Th>
                  <Table.Th c="dimmed" fw={500} fz="xs">Value</Table.Th>
                </Table.Tr>
              </Table.Thead>
              <Table.Tbody>
                {Object.entries(input).map(([key, value]) => {
                  ...
                  return (
                    <Table.Tr key={key}>
                      <Table.Td>
                        <Text fw={500} ff="monospace" size="sm">{key}</Text>
                      </Table.Td>
                      <Table.Td>
                        <TypeBadge
                          type={parsed.type}
                          isHoverable={parsed.isHoverable}
                          onMouseEnter={parsed.isHoverable && parsed.hoverRef ? () => handleRefHover(parsed.hoverRef!) : undefined}
                          onMouseLeave={parsed.isHoverable ? () => handleRefHover(null) : undefined}
                        />
                      </Table.Td>
                      <Table.Td>
                        {parsed.displayValue}
...
export const WorkflowStepOutput: React.FunctionComponent<WorkflowStepPanelProps & { showOutputs?: boolean }> = ({
  stepId,
  showOutputs = false,
}) => {
  const node = useWorkflowNode(stepId);
          ...
          {schemaLoading ? (
            ...
            <Card.Section>
              <Table verticalSpacing={4} horizontalSpacing="xs">
                <Table.Thead>
                  <Table.Tr>
                    <Table.Th c="dimmed" fw={500} fz="xs">Property</Table.Th>
                    <Table.Th c="dimmed" fw={500} fz="xs">Type</Table.Th>
                  </Table.Tr>
                </Table.Thead>
                <Table.Tbody>
                ...
                <Table verticalSpacing={4} horizontalSpacing="xs">
                  <Table.Thead>
                    <Table.Tr>
                      ...
                      <Table.Th c="dimmed" fw={500} fz="xs">Source</Table.Th>
                    </Table.Tr>
                  </Table.Thead>
                  <Table.Tbody>
                    {Object.entries(outputs).map(([key, value]: [string, any]) => (
                      <Table.Tr key={key}>
                        ...
                        <Table.Td>
                          {value?.from && (
                            <Group gap="xs">
                              <PikkuBadge type="label">{value.from}</PikkuBadge>
                              {value.name && <Text size="sm" ff="monospace">{value.name}</Text>}
                              {value.expression && <Text size="sm" ff="monospace">{value.expression}</Text>}
                              {value.path && (
                                <>
                                  <Text size="sm" c="dimmed">.</Text>
                                  <Text size="sm" ff="monospace">{value.path}</Text>
                                </>
                              )}
                            </Group>
                          )}
                          {!value?.from && <Code>{JSON.stringify(value, null, 2)}</Code>}
                        </Table.Td>
                      </Table.Tr>
                    ))}
                  </Table.Tbody>
...
export const WorkflowStepBranches: React.FunctionComponent<WorkflowStepPanelProps> = ({
  stepId,
}) => {
  const node = useWorkflowNode(stepId);
  const flowType = node?.flow;

  const flowLabel = flowType === "branch" ? "Branches"
    : flowType === "switch" ? "Cases"
    : flowType === "parallel" ? "Children"
    : flowType === "fanout" ? "Fanout"
    : "Flow";
            ...
            {flowType === "branch" && node?.branches ? (
              <Stack gap="xs">
                {node.branches.map((branch: any, index: number) => (
                  <Box key={index} p="sm" style={{ border: "1px solid var(--mantine-color-gray-3)", borderRadius: "var(--mantine-radius-sm)" }}>
                    <Group gap="xs" mb="xs">
                      <PikkuBadge type="label">{index === 0 ? "if" : "else if"}</PikkuBadge>
                      <Text size="sm" ff="monospace">{branch.condition?.expression || "true"}</Text>
                    </Group>
                    {branch.entry && (
                      <Text size="xs" c="dimmed">Entry: <Text span ff="monospace">{branch.entry}</Text></Text>
                    )}
                  </Box>
                ))}
              </Stack>
                ...
                {node.cases.map((caseItem: any, index: number) => (
                  <Box key={index} p="sm" style={{ border: "1px solid var(--mantine-color-gray-3)", borderRadius: "var(--mantine-radius-sm)" }}>
                    <Group gap="xs" mb="xs">
                      ...
                      <Text size="sm" ff="monospace">{caseItem.value}</Text>
                    </Group>
                    {caseItem.entry && (
                      <Text size="xs" c="dimmed">Entry: <Text span ff="monospace">{caseItem.entry}</Text></Text>
                    )}
                  </Box>
                ))}
                {node.defaultEntry && (
                  <Box p="sm" style={{ border: "1px solid var(--mantine-color-gray-3)", borderRadius: "var(--mantine-radius-sm)" }}>
                    <Group gap="xs" mb="xs">
                      <PikkuBadge type="label" color="gray">default</PikkuBadge>
                    </Group>
                    <Text size="xs" c="dimmed">Entry: <Text span ff="monospace">{node.defaultEntry}</Text></Text>
                  </Box>
                )}
              </Stack>
              ...
              <Stack gap="xs">
                {node.children.map((childId: string, index: number) => (
                  <Text key={index} size="sm" ff="monospace">{childId}</Text>
                ))}
              </Stack>
            ) : flowType === "fanout" ? (
              <Stack gap="xs">
                {node?.sourceVar && (
                  <Group gap="xs">
                    ...
                    <Text size="sm" ff="monospace">{node.sourceVar}</Text>
                  </Group>
                )}
                {node?.itemVar && (
                  <Group gap="xs">
                    <Text size="sm" c="dimmed">Item:</Text>
                    <Text size="sm" ff="monospace">{node.itemVar}</Text>
                  </Group>
                )}
                {node?.mode && (
                  <Group gap="xs">
                ...
                {node?.childEntry && (
                  <Group gap="xs">
                    <Text size="sm" c="dimmed">Child Entry:</Text>
                    <Text size="sm" ff="monospace">{node.childEntry}</Text>
                  </Group>
                )}
              </Stack>
            ) : (
