...
const COLUMNS = [
  {
    ...
    render: (entry: CliProgramEntry) => (
      <Text fw={500} ff="monospace" truncate>{entry.wireId}</Text>
    ),
  },
  {
    key: "description",
  ...
  },
  {
    ...
    render: (entry: CliProgramEntry) => (
      <Group gap={6}>
        <PikkuBadge type="dynamic" badge="functions" value={entry.commandCount} />
      </Group>
    ),
  },
];
  ...
  return (
    <PanelProvider>
      <ResizablePanelLayout
        header={<DetailPageHeader icon={Terminal} category="CLI" docsHref="https://pikkujs.com/docs/cli" />}
        hidePanel
      >
        <TableListPage
          title="CLI"
          ...
          onRowClick={(entry) => navigate(`/apis/cli?id=${encodeURIComponent(entry.wireId)}`)}
