    ...
    if (ts.isStringLiteral(element)) {
      logger.critical(
        ...
        `AI agent '${agentName}' agents array contains a string literal '${element.text}'. ` +
          `Use an agent reference instead (e.g., import the pikkuAgent variable).`
      )
      continue
    }
    ...
    if (ts.isIdentifier(element)) {
      ...
      logger.critical(
        ...
        `AI agent '${agentName}' agents array contains identifier '${element.text}' ` +
          `that could not be resolved to a pikkuAgent.`
      )
    }
  }
...
function resolveIdentifierToAgentName(
  ...
  if (ts.isVariableDeclaration(decl) && decl.initializer) {
    if (
      ts.isCallExpression(decl.initializer) &&
      ts.isIdentifier(decl.initializer.expression) &&
      decl.initializer.expression.text === 'pikkuAgent'
    ) {
      if (ts.isIdentifier(decl.name)) {
        return decl.name.text
      }
...
export const addAgent: AddWiring = (logger, node, checker, state, options) => {
  ...
  if (!ts.isIdentifier(expression) || expression.text !== 'pikkuAgent') {
    return
  }
  ...
  if (ts.isObjectLiteralExpression(firstArg)) {
    ...
    const agentMiddleware = resolveAgentMiddleware(state, obj, checker)
