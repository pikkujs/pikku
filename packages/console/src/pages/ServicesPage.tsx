...
export const ServicesPage: React.FunctionComponent = () => {
  ...
  const services = useMemo((): ServiceItem[] => {
    const serviceMap = new Map<string, { funcCount: number; functions: string[] }>();
    meta.functions?.forEach((func: any) => {
      if (func.services?.services) {
        for (const svc of func.services.services) {
          const existing = serviceMap.get(svc) || { funcCount: 0, functions: [] };
          existing.funcCount++;
  ...
  return (
    <PanelProvider>
      <ResizablePanelLayout
        header={<DetailPageHeader icon={Server} category="Services" docsHref="https://pikkujs.com/docs/services" />}
        hidePanel
      >
        <TableListPage
          title="Services"
