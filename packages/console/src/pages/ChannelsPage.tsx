...
const COLUMNS = [
  {
    ...
    render: (ch: ChannelEntry) => (
      <>
        <Text fw={500} truncate>{ch.name}</Text>
        <Text size="xs" c="dimmed" truncate>{ch.route}</Text>
      </>
    ),
  },
  {
  ...
  return (
    <PanelProvider>
      <ResizablePanelLayout
        header={<DetailPageHeader icon={Radio} category="Channels" docsHref="https://pikkujs.com/docs/channels" />}
        hidePanel
      >
        <TableListPage
          title="Channels"
          ...
          columns={COLUMNS}
          getKey={(ch) => ch.name}
          onRowClick={(ch) => navigate(`/apis/channels?id=${encodeURIComponent(ch.name)}`)}
          searchPlaceholder="Search channels..."
          searchFilter={(ch, q) =>
            ch.name.toLowerCase().includes(q) || ch.route.toLowerCase().includes(q)
          }
