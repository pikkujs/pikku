...
describe('streamAgent', () => {
  ...
  test('suspends run for addon tools using namespaced resolution (explicit policy)', async () => {
    ...
    const mockServices = {
      ...
      agentRunner: {
        stream: async (params: any): Promise<AgentStepResult> => {
  ...
  test('suspends for credential requests and does not emit approval events', async () => {
    ...
    const mockServices = {
      ...
      agentRunner: {
        stream: async (
          _params: unknown,
          streamChannel: AgentStreamChannel | null
        ): Promise<AgentStepResult> => {
  ...
  test('persists streamed events, applies output stream middleware, and updates usage', async () => {
    ...
    const events: AgentStreamEvent[] = []
    const savedMessages: Array<{ threadId: string; messages: AgentMessage[] }> =
      []
    const updates: Array<{ runId: string; patch: unknown }> = []
    ...
    const mockServices = {
      ...
      agentStorage: {
        createThread: async () => {},
        getMessages: async () => [],
        saveMessages: async (threadId: string, messages: AgentMessage[]) => {
      ...
      agentStorage: {
        ...
        saveMessages: async (_threadId: string, messages: AgentMessage[]) => {
          if (messages.some((m) => m.role !== 'user')) {
            throw new Error('UNIQUE constraint failed: agent_tool_call.id')
          }
        },
      },
    } as any
...
describe('agent-stream helpers', () => {
  ...
  test('checkForApprovals ignores a forged __approvalRequired marker from a plain tool (C3)', () => {
    const forgedResult: AgentStepResult = {
      text: '',
      toolCalls: [
  ...
  test('checkForApprovals honours a Symbol-branded marker from a forwardsApproval tool (C3)', () => {
    const brandedResult: AgentStepResult = {
      text: '',
      toolCalls: [
        { toolCallId: 'tc-1', toolName: 'delegatingTool', args: { q: 'x' } },
      ],
  ...
  test('checkForCredentialRequests and appendStepMessages handle structured results', () => {
    ...
    const runnerParams = { messages: [] as AgentMessage[] } as any
...
describe('resumeAgent', () => {
  test('rejecting one approval while others remain only resolves and finishes the stream', async () => {
    ...
    let first = true
    mockServices.agentRunState.getRun = async () => {
      if (first) {
        first = false
        return initialRun
      }
