...
import { Globe, Radio, FunctionSquare, Clock, ListOrdered, Terminal, Cpu, Zap } from "lucide-react";
    ...
    <Stack gap="lg">
      <Box>
        <Group gap="xs">
          ...
          <Text size="lg" ff="monospace" fw={600}>{route}</Text>
        </Group>
        {metadata?.summary && (
      ...
      {((metadata?.params && metadata.params.length > 0) || (metadata?.query && metadata.query.length > 0)) && (
        <Group gap="xs">
          {metadata?.params?.map((p: string) => (
            <PikkuBadge key={`p-${p}`} type="dynamic" badge="param" value={`:${p}`} />
          ))}
          {metadata?.query?.map((q: string) => (
            <PikkuBadge key={`q-${q}`} type="dynamic" badge="query" value={`?${q}`} />
          ))}
        </Group>
      )}
...
export const ChannelConfiguration: React.FunctionComponent<WiringPanelProps> = ({
  wireId,
  metadata = {},
}) => {
  ...
  const hasAuth = metadata?.auth !== false;
  const messageWirings = metadata?.messageWirings as Record<string, Record<string, any>> | undefined;
  ...
  return (
    <Stack gap="lg">
      <Box>
        ...
        {metadata?.route && (
          <Text size="sm" c="dimmed" mt={4}>{metadata.route}</Text>
        )}
        {metadata?.summary && (
          <Text size="sm" c="dimmed" mt={4}>{metadata.summary}</Text>
        )}
      ...
      >
        ...
        <FunctionLink pikkuFuncId={metadata?.connect?.pikkuFuncId} label="Connect Handler" />
        <FunctionLink pikkuFuncId={metadata?.disconnect?.pikkuFuncId} label="Disconnect Handler" />
        <FunctionLink pikkuFuncId={metadata?.message?.pikkuFuncId} label="Message Handler" />
          ...
          <Box>
            ...
            <Stack gap="sm">
              {Object.entries(messageWirings).map(([category, actions]) => (
                <Box key={category}>
                  <Text size="sm" fw={600} mb={4}>{category}</Text>
                  <Group gap={6}>
                    {Object.entries(actions).map(([actionName, actionData]) => (
                      <PikkuBadge
                        ...
                        badge="actions"
                        value={actionName}
                        style={{ cursor: actionData?.pikkuFuncId ? "pointer" : undefined }}
                        onClick={actionData?.pikkuFuncId ? () => navigateInPanel("function", actionData.pikkuFuncId, actionName, actionData) : undefined}
                      />
                    ))}
                  </Group>
                </Box>
    ...
    <Stack gap="lg">
      <Box>
        <Group gap="xs">
          ...
          <Text size="lg" ff="monospace" fw={600}>{wireId}</Text>
        </Group>
      </Box>
...
export const SchedulerConfiguration: React.FunctionComponent<WiringPanelProps> = ({
  wireId,
  metadata = {},
}) => {
  const middleware = metadata?.middleware || [];
    ...
    <Stack gap="lg">
      ...
      {metadata?.schedule && (
        <Box>
          ...
          <Text size="xs" c="dimmed" ff="monospace" mt={2}>{metadata.schedule}</Text>
        </Box>
      )}
    ...
    <Stack gap="lg">
      ...
      <Group gap="xs">
        ...
        {metadata?.concurrency !== undefined && (
          <PikkuBadge type="dynamic" badge="concurrency" value={metadata.concurrency} variant="outline" color="gray" />
        )}
        {permissions.length > 0 && (
          <PikkuBadge type="flag" flag="permissioned" />
        )}
          ...
          <Box>
            ...
            <Group gap={6}>
              {Object.entries(config).map(([key, value]) => (
                <PikkuBadge key={key} type="label" size="sm" variant="outline" color="gray">
                  {key}: {String(value)}
                </PikkuBadge>
              ))}
            </Group>
...
const CliOptionsTable: React.FunctionComponent<{ options: Record<string, any> }> = ({ options }) => {
  ...
  return (
    <Table verticalSpacing={4} horizontalSpacing="xs">
      <Table.Thead>
        <Table.Tr>
          <Table.Th c="dimmed" fw={500} fz="xs">Option</Table.Th>
          <Table.Th c="dimmed" fw={500} fz="xs">Description</Table.Th>
          <Table.Th c="dimmed" fw={500} fz="xs">Default</Table.Th>
        </Table.Tr>
      </Table.Thead>
      <Table.Tbody>
...
const CliCommandTree: React.FunctionComponent<{ commands: Record<string, any>; depth?: number }> = ({ commands, depth = 0 }) => {
  const { navigateInPanel } = usePanelContext();
      ...
      {Object.entries(commands).map(([name, cmd]) => (
        <Box key={name} ml={depth * 16}>
          <Group gap={6} mb={4}>
            ...
            {cmd.isDefault && (
              <PikkuBadge type="label" size="sm" variant="light" color="gray">default</PikkuBadge>
            )}
            {cmd.pikkuFuncId && (
              <PikkuBadge
                type="label"
                ...
                onClick={() => navigateInPanel("function", cmd.pikkuFuncId, cmd.pikkuFuncId, cmd)}
              >
                {cmd.pikkuFuncId}
              </PikkuBadge>
            )}
  ...
  return (
    <Stack gap="lg">
      <Box>
        <Group gap="xs">
          <Terminal size={20} />
          <Text size="lg" ff="monospace" fw={600}>{program}</Text>
        </Group>
        {metadata?.summary && (
          <Text size="sm" c="dimmed" mt={4}>{metadata.summary}</Text>
        )}
      ...
      <Group gap="xs">
        ...
        {metadata?.defaultRenderName && (
          <PikkuBadge type="dynamic" badge="format" value={metadata.defaultRenderName} />
        )}
      </Group>
...
export const McpConfiguration: React.FunctionComponent<WiringPanelProps> = ({
  ...
  const middleware = metadata?.middleware || [];
  const permissions = metadata?.permissions || [];
  const args = metadata?.arguments as Array<{ name: string; description?: string; required?: boolean }> | undefined;
    ...
    <Stack gap="lg">
      ...
      {metadata?.uri && (
        <Box>
          ...
          <Text size="sm" ff="monospace">{metadata.uri}</Text>
        </Box>
      )}
        ...
        {args && args.length > 0 && (
          <Box>
            ...
            <Table verticalSpacing={4} horizontalSpacing="xs">
              <Table.Thead>
                <Table.Tr>
                  <Table.Th c="dimmed" fw={500} fz="xs">Name</Table.Th>
                  <Table.Th c="dimmed" fw={500} fz="xs">Description</Table.Th>
                  <Table.Th c="dimmed" fw={500} fz="xs">Required</Table.Th>
                </Table.Tr>
              </Table.Thead>
              <Table.Tbody>
                {args.map((arg) => (
                  <Table.Tr key={arg.name}>
                    <Table.Td>
                      <Text size="sm" ff="monospace">{arg.name}</Text>
                    </Table.Td>
                    <Table.Td>
                      <Text size="sm">{arg.description || ""}</Text>
                    </Table.Td>
                    <Table.Td>
                      {arg.required && <PikkuBadge type="flag" flag="required" />}
                    </Table.Td>
                  </Table.Tr>
                ))}
              </Table.Tbody>
...
export const TriggerConfiguration: React.FunctionComponent<WiringPanelProps> = ({
  wireId,
  metadata = {},
}) => {
  const middleware = metadata?.middleware || [];
...
export const TriggerSourceConfiguration: React.FunctionComponent<WiringPanelProps> = ({
  wireId,
  metadata = {},
}) => {
  return (
    <Stack gap="lg">
      ...
      <Group gap="xs">
        ...
        {metadata?.packageName && (
          <PikkuBadge type="dynamic" badge="package" value={metadata.packageName} />
        )}
      </Group>
