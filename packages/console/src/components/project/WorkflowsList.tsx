...
const COLUMNS = [
  ...
  },
  {
    ...
    render: (w: Workflow) => (
      <PikkuBadge type="dynamic" badge="steps" value={Object.keys(w.nodes || {}).length} />
    ),
  },
];
...
export const WorkflowsList: React.FunctionComponent<WorkflowsListProps> = ({ workflows }) => {
  ...
  return (
    <TableListPage
      ...
      searchFilter={(w, q) =>
        (w.name?.toLowerCase().includes(q) || w.pikkuFuncId?.toLowerCase().includes(q)) ?? false
      }
