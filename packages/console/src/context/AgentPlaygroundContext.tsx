...
const AgentPlaygroundContext = createContext<AgentPlaygroundContextType | undefined>(undefined);

export const useAgentPlayground = () => {
  const context = useContext(AgentPlaygroundContext);
  if (!context) {
    throw new Error("useAgentPlayground must be used within AgentPlaygroundProvider");
  }
  return context;
};
...
export const AgentPlaygroundProvider: React.FunctionComponent<AgentPlaygroundProviderProps> = ({
  children,
  agentId,
}) => {
  ...
  const threads: AgentThread[] = (dbThreads as any[] || []).map(mapDbThreadToAgentThread);
