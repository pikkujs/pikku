...
>({
  ...
  func: async ({}, { name, nodes, workflowDescription, entryNodeIds }) => {
    ...
    const canonical = JSON.stringify({ nodes, entryNodeIds }, Object.keys({ nodes, entryNodeIds }).sort())
    const graphHash = createHash('sha256').update(canonical).digest('hex').slice(0, 16)
