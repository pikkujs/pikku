...
import ReactFlow, { useNodesState, useEdgesState, NodeTypes, EdgeTypes, MarkerType, Background, BackgroundVariant, useReactFlow, ReactFlowProvider } from "reactflow";
import { Box, Drawer, Group, Text, Alert } from "@mantine/core";
import { AlertTriangle, GitBranch, History } from "lucide-react";
import { CanvasDrawerProvider, useCanvasDrawerContext } from "@/context/DrawerContext";
import { WorkflowProvider, useWorkflowContext } from "@/context/WorkflowContext";
import { useWorkflowRunContextSafe, useWorkflowRunContext } from "@/context/WorkflowRunContext";
...
function filterWiresForRun(wires: any, runWire: { type: string; id?: string }): any {
  ...
  if (wiresKey === "mcp" || runWire.type === "mcp") {
    ...
    if (wires.mcp[subType]) {
      filtered[subType] = wires.mcp[subType].filter((w: any) => (w.name || w.uri) === matchId);
    }
    return { mcp: filtered };
  }
  ...
  return (
    <Box style={{ width: "100%", height: "100%" }}>
      ...
      <ReactFlow
        ...
        edgeTypes={edgeTypes}
        defaultEdgeOptions={{
          style: { stroke: "#c0c0c0", strokeWidth: 1.5, strokeDasharray: "6 4" },
          animated: true,
          markerEnd: { type: MarkerType.ArrowClosed, color: "#c0c0c0", width: 16, height: 16 },
        }}
        zoomOnScroll={true}
...
const WorkflowRunsPanel: React.FunctionComponent<{ workflowName: string }> = ({ workflowName }) => {
  const { selectedRunId, setSelectedRunId, setIsCreatingRun } = useWorkflowRunContext();
...
const WorkflowCanvasContent: React.FunctionComponent<WorkflowCanvasProps> = ({
  ...
  const { panels, activePanel, openWorkflow, setActivePanel } = usePanelContext();
  ...
  const header = (
    <>
      <DetailPageHeader
        ...
        subtitle={workflow.source ? <PikkuBadge type="dynamic" badge="source" value={workflow.source} /> : undefined}
      />
      {isComplex && (
        <Alert
        ...
        >
          <Group justify="space-between" align="center">
            <Text size="sm">This is a complex workflow. The visual representation may not be accurate.</Text>
            <Text size="sm" c="yellow.7" style={{ cursor: "pointer", textDecoration: "underline" }}>
              Learn more
            </Text>
          </Group>
        </Alert>
      ...
      {runContext?.isVersionMismatch && (
        <Alert
          ...
          styles={{ root: { borderBottom: "1px solid var(--mantine-color-gray-3)" } }}
        >
          <Text size="sm">Viewing historical version — workflow definition has changed since this run</Text>
        </Alert>
      )}
    </>
  );

  const baseWorkflow = runContext?.isVersionMismatch && runContext?.historicalWorkflow
  ...
  const canvasWorkflow = useMemo(() => {
    ...
    return { ...baseWorkflow, wires: filterWiresForRun(baseWorkflow.wires, runWire) };
  }, [baseWorkflow, runContext?.runData?.wire]);

  const runsPanel = runContext ? <WorkflowRunsPanel workflowName={workflowName} /> : undefined;
    ...
    <>
      ...
      >
        <Box style={{ height: "100%", position: "relative" }}>
          <WorkflowCanvasInner workflow={canvasWorkflow} onPaneClick={handlePaneClick} />
        </Box>
      </ThreePaneLayout>
      {canvasDrawer && (
        <Box
...
export const WorkflowCanvas: React.FunctionComponent<WorkflowCanvasProps> = (props) => {
  return (
