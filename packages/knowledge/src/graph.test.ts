...
describe('buildKnowledgeGraph', () => {
  ...
  test('a legacy slices/ sorts and reads as the milestones section it is', () => {
    // Otherwise the rename demotes every bundle written before it: `slices/` is
    // a section the profile no longer names, so it would sort last, behind
    // `questions`, and lose the description the navigator shows under it.
    const graph = buildKnowledgeGraph([
      note('knowledge/questions/open.md', '---\ntype: question\n---\nq'),
