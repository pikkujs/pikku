...
const QueuesTable: React.FunctionComponent<{ items: QueueItem[]; loading?: boolean }> = ({ items, loading }) => {
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      render: (item: QueueItem) =>
        item.concurrency ? (
          <PikkuBadge type="dynamic" badge="concurrency" value={item.concurrency} />
        ) : null,
    },
  ], []);
...
export const QueuesPage: React.FunctionComponent = () => {
  ...
  return (
    <PanelProvider>
      <ResizablePanelLayout header={<DetailPageHeader icon={ListOrdered} category="Queues" docsHref="https://pikkujs.com/docs/queues" />} showTabs={false} hidePanel={!loading && items.length === 0} emptyPanelMessage="Select a queue worker to view its details">
