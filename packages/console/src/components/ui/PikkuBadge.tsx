...
export const PikkuBadge: React.FunctionComponent<PikkuBadgeProps> = (props) => {
  ...
  if (type === 'flag') {
    const { flag, ...badgeProps } = rest as { flag: string } & Omit<
      BadgeProps,
      'children'
    >
  ...
  if (type === 'dynamic') {
    const { badge, value, ...badgeProps } = rest as {
      badge: string
      value: string | number
    } & Omit<BadgeProps, 'children'>
  ...
  const { value, ...badgeProps } = rest as { value: string } & Omit<
    BadgeProps,
    'children'
  >
