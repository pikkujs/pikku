...
// middleware is a no-op while the secret is unset.
//
...
export const serializeAuthGen = (
  ...
  // The generated session middleware is always emitted, at `lowest` priority. An
  // app's own registration defaults to `medium`, so it resolves the session
  // first and this one short-circuits on `if (session) next()`. That is what
  // pikkujs/pikku#754 needed: the CLI used to detect a user registration and
  // skip generating at all, which silently dropped anything emitted alongside.
  const emitStatefulSession = !stateless

  const wiring: string[] = [
    '// AUTO-GENERATED by pikku CLI — do not edit',
    '',
    emitStatefulSession
      ? `import { pikkuSessionlessFunc } from '${leaf('function')}'\nimport { wireHTTPRoutes } from '${leaf('http')}'\nimport { addGlobalMiddleware${includeConsoleToken ? ', authBearer' : ''} } from '@pikku/core/middleware'`
      : `import { pikkuSessionlessFunc } from '${leaf('function')}'\nimport { wireHTTPRoutes } from '${leaf('http')}'`,
    emitStatefulSession
  ...
  if (emitStatefulSession) {
    ...
    wiring.push(
      `addGlobalMiddleware([`,
      `  betterAuthSession({ priority: 'lowest' }),`,
      ...(includeConsoleToken ? consoleTokenMiddlewareLines() : []),
      `])`,
      ''
    )
  }
  ...
  if (stateless) {
    ...
    const middleware: string[] = [
      ...
      '',
      `addGlobalMiddleware([`,
      `  betterAuthStatelessSession({ priority: 'lowest' }),`,
      ...(includeConsoleToken ? consoleTokenMiddlewareLines() : []),
      `])`,
      '',
    ]
