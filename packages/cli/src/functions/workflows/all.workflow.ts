...
export const allWorkflow = pikkuWorkflowComplexFunc<void, void>({
  ...
  func: async ({ logger, config, getInspectorState }, _data, { workflow }) => {
    ...
    // exist when it is written. These three are pure — they read `config` and
    ...
    // `#pikku/secrets` that resolves but is missing `defineSecret`.
    await Promise.all([
      workflow.do(
    ...
    // These write the addon-safe subset of their leaf when the project is an
    // addon, so they run either way. Skipping them for an addon only meant the
    // bootstrap pass's copy was never rewritten, and an addon generated by an
    // older CLI kept wiring leaves that no longer compiled against the current
    // function leaf.
