...
Feature: Dynamic Workflows via Todo Agent (API)
  ...
  list, and execute workflows using its existing tools.
  ...
  Scenario: Explicit workflow creation with tool names
    When I send the agent "todoAgent" the message "Use create_workflow to create a workflow called 'add-and-list' with two nodes: first node calls todos:addTodo with input title 'Workflow task', second node calls todos:listTodos. The first node should flow to the second."
    Then the agent response should contain "add-and-list"

    When I send the agent "todoAgent" the message "Use list_workflows to show my workflows"
    Then the agent response should contain "add-and-list"

    When I send the agent "todoAgent" the message "Use execute_workflow to run the 'add-and-list' workflow"
  ...
  Scenario Outline: Natural language workflow - <style>
    When I send the agent "todoAgent" the message "<create_prompt>"
    ...
    Then the todo "Sleep test item" should be in the list
    And the todo "Sleep test item" should be completed
