  ...
  func: async ({ logger, cliConfig, getInspectorState }) => {
    ...
    for (const [programName, programMeta] of Object.entries(
      ...
      const variants = [
        ...
        { suffix: '-rpc', mode: 'rpc' as const },
      ]

      for (const { suffix, mode } of variants) {
        // Skip RPC mode if rpcWiringsFile is not configured
        if (mode === 'rpc' && !config.rpcWiringsFile) {
          logger.info({
            message: `Skipping RPC CLI bootstrap for ${programName} - depends on rpcWiringsFile config`,
            type: 'skip',
          })
          continue
        }
