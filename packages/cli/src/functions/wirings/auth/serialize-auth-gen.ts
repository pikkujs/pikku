...
// middleware is a no-op while the secret is unset. Emitted alongside the session
// middleware in one addGlobalMiddleware call. This used to be an
// addHTTPMiddleware('*') call and had to be a single one, because the inspector
// keys route-middleware groups by pattern and a second '*' registration would
...
export const serializeAuthGen = (
  ...
  if (emitStatefulSession) {
    ...
    if (includeConsoleToken) {
      ...
      wiring.push(`addGlobalMiddleware([betterAuthSession()])`, '')
    }
  }
  ...
  if (stateless) {
    ...
    // Global, not `addHTTPMiddleware('*')`.
    //
    // Both entries resolve a session from whatever request the call arrived on;
    // neither is an HTTP routing concern. Registered as HTTP middleware they ran
    // for HTTP wirings alone, so an MCP tool fronting a session-requiring
    // function could only answer 'Authentication required' however the caller
    // authenticated — the cookie was sent and never looked at.
    //
    // Wirings that carry no request are unaffected: every one of these opens
    // with `if (!http?.request) return`, so queue, scheduler and CLI invocations
    // pass straight through as before.
    //
    // This also retires the displacement hazard that kept both entries in one
    // call — global middleware is a list rather than a map keyed by route
    // pattern, so a second registration appends instead of overwriting.
