...
export async function runFabricValidate(
  ...
  const REQUIRED_GITIGNORE_ENTRIES: Array<{
    name: string
    patterns: string[]
    reason: string
  }> = [
    ...
    },
    {
      ...
      reason:
        'generated TypeScript files (by pikku and TanStack Start) should not be committed — they are regenerated at build time',
    },
    {
      name: '*.gen.js',
      patterns: ['*.gen.js', '**/*.gen.js'],
      reason:
        'generated JavaScript files (by pikku and TanStack Start) should not be committed — they are regenerated at build time',
    },
  ]
  try {
    ...
    for (const entry of REQUIRED_GITIGNORE_ENTRIES) {
      ...
      if (!covered) {
        w(
          `${entry.name.replace(/[^a-z0-9]/gi, '-')}-not-gitignored`,
          `${entry.name} is not listed in .gitignore — ${entry.reason}`,
          gitignorePath,
          `Add "${entry.name}" to .gitignore`
        )
      }
    }
  } catch {
    w(
      'gitignore-missing',
      '.gitignore not found at project root — generated artifacts (.pikku, .pikku-runtime, .opencode) may be committed accidentally',
      gitignorePath,
      'Create .gitignore and add ".pikku", ".pikku-runtime", and ".opencode" to it'
    )
  }
