...
export const WorkflowPageClient: React.FunctionComponent = () => {
  ...
  return (
    <PanelProvider>
      <WorkflowRunProvider workflowName={workflowId} currentGraphHash={(workflow as any).graphHash} workflowNodes={(workflow as any).nodes}>
        <Box h="100vh" style={{ display: "flex", flexDirection: "column" }}>
          <Box style={{ flex: 1, overflow: "auto" }}>
            <WorkflowCanvas
              workflow={workflow}
              items={workflowItems}
              onItemSelect={(name) => navigate(`/workflow?id=${encodeURIComponent(name)}`)}
            />
          </Box>
        </Box>
      </WorkflowRunProvider>
