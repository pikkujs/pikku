...
import { Stack, Text, Group, Table, Card, Box, Code, Anchor } from "@mantine/core";
...
export const WorkflowHeader: React.FunctionComponent<WorkflowPanelProps> = ({
  ...
  return (
    <Box>
      <Group gap="xs">
        <GitBranch size={20} />
        <Text size="lg" ff="monospace" fw={600}>{workflow?.name || workflowId}</Text>
        {workflow?.graphHash && (
          <Text size="xs" ff="monospace" c="dimmed">#{workflow.graphHash}</Text>
        )}
      </Group>
      <Text size="sm" c="dimmed" mt={4}>{workflow?.summary || "No summary"}</Text>
    </Box>
  );
};
...
export const WorkflowConfiguration: React.FunctionComponent<WorkflowPanelProps> = ({
  workflowId,
}) => {
  const { workflow } = useWorkflowContext();
    ...
    <Stack gap="lg">
      ...
      {workflow?.pikkuFuncId && (
        <Box>
          ...
          <Text size="sm" ff="monospace">{workflow.pikkuFuncId}</Text>
        </Box>
      )}
            ...
            <Table verticalSpacing={4} horizontalSpacing="xs" highlightOnHover>
              ...
              <Table.Tbody>
                {Object.entries(nodes).map(([nodeId, node]: [string, any]) => (
                  ...
                  >
                    <Table.Td>
                      <Text fw={500} ff="monospace" size="sm">{node.stepName || nodeId}</Text>
                      {node.stepName && node.stepName !== nodeId && (
                        <Text size="xs" c="dimmed">{nodeId}</Text>
                      )}
                    </Table.Td>
                    <Table.Td>
                      {node.flow && <PikkuBadge type="label">{node.flow}</PikkuBadge>}
                      {node.rpcName && <PikkuBadge type="label" color="green">RPC</PikkuBadge>}
                    </Table.Td>
                  </Table.Tr>
                ))}
              </Table.Tbody>
          ...
          <Table verticalSpacing={4} horizontalSpacing="xs" highlightOnHover>
            <Table.Thead>
              <Table.Tr>
                <Table.Th c="dimmed" fw={500} fz="xs">Name</Table.Th>
                <Table.Th c="dimmed" fw={500} fz="xs">Type</Table.Th>
                <Table.Th c="dimmed" fw={500} fz="xs">Status</Table.Th>
                <Table.Th c="dimmed" fw={500} fz="xs">Runs</Table.Th>
              </Table.Tr>
            </Table.Thead>
            <Table.Tbody>
                ...
                return (
                  ...
                  >
                    ...
                    <Table.Td>
                      {node.flow && <PikkuBadge type="label">{node.flow}</PikkuBadge>}
                      {node.rpcName && (
                        <PikkuBadge type="label" color="green">{node.rpcName}</PikkuBadge>
                      )}
                    </Table.Td>
                    <Table.Td>
                    ...
                    <Table.Td>
                      <Text size="sm" ff="monospace">{step.attemptCount}</Text>
                    </Table.Td>
                  </Table.Tr>
                );
              })}
        ...
        {hasContext ? (
          <Card.Section>
            <Table verticalSpacing={4} horizontalSpacing="xs">
              <Table.Thead>
                <Table.Tr>
                  <Table.Th c="dimmed" fw={500} fz="xs">Variable</Table.Th>
                  <Table.Th c="dimmed" fw={500} fz="xs">Type</Table.Th>
                  <Table.Th c="dimmed" fw={500} fz="xs">Default</Table.Th>
                </Table.Tr>
              </Table.Thead>
              <Table.Tbody>
                {Object.entries(context).map(([varName, varDef]: [string, any]) => (
                  <Table.Tr key={varName}>
                    <Table.Td>
                      <Text fw={500} ff="monospace" size="sm">{varName}</Text>
                    </Table.Td>
                    <Table.Td>
                      <PikkuBadge type="label">{varDef.type || "unknown"}</PikkuBadge>
                    </Table.Td>
                    <Table.Td>
...
export const WorkflowRunOverview: React.FunctionComponent<WorkflowPanelProps> = ({
  workflowId,
}) => {
  const runContext = useWorkflowRunContextSafe();
  ...
  const endTime = runData.status === "completed" || runData.status === "failed" || runData.status === "cancelled"
    ? runData.updatedAt
    : undefined;
    ...
    <Stack gap="md">
        ...
        <Card withBorder radius="md" padding={0}>
          <Card.Section>
            <Table verticalSpacing={4} horizontalSpacing="xs">
              <Table.Tbody>
                <Table.Tr>
                  <Table.Td><Text size="sm" c="dimmed">Run ID</Text></Table.Td>
                  <Table.Td><Text size="sm" ff="monospace">{runData.id}</Text></Table.Td>
                </Table.Tr>
                <Table.Tr>
                  <Table.Td><Text size="sm" c="dimmed">Started</Text></Table.Td>
                  <Table.Td><Text size="sm" ff="monospace">{formatTimestamp(runData.createdAt)}</Text></Table.Td>
                </Table.Tr>
                {endTime && (
                  <Table.Tr>
                    <Table.Td><Text size="sm" c="dimmed">Ended</Text></Table.Td>
                    <Table.Td><Text size="sm" ff="monospace">{formatTimestamp(endTime)}</Text></Table.Td>
                  </Table.Tr>
                )}
                {duration && (
                  <Table.Tr>
                ...
                {runData.wire && (
                  <Table.Tr>
                    <Table.Td><Text size="sm" c="dimmed">Wire</Text></Table.Td>
                    <Table.Td>
                      <Group gap={4}>
                        <PikkuBadge type="label" size="sm" color={wiringTypeColor(runData.wire.type)}>{runData.wire.type}</PikkuBadge>
                        {runData.wire.id && <Text size="sm" ff="monospace">{runData.wire.id}</Text>}
                      </Group>
                    </Table.Td>
                  </Table.Tr>
                )}
                {runData.graphHash && (
                  <Table.Tr>
                    <Table.Td><Text size="sm" c="dimmed">Graph Hash</Text></Table.Td>
                    <Table.Td><Text size="sm" ff="monospace">{runData.graphHash}</Text></Table.Td>
                  </Table.Tr>
                )}
              </Table.Tbody>
            </Table>
      ...
      {runData.error && (
        <Stack gap={6}>
          ...
          <Card withBorder radius="md" padding={0}>
            <Card.Section p="md">
              <Text size="sm" ff="monospace" c="red">{typeof runData.error.message === 'string' ? runData.error.message : JSON.stringify(runData.error.message ?? runData.error, null, 2)}</Text>
            </Card.Section>
          </Card>
        </Stack>
      )}
