...
export const ProjectFunctions: React.FunctionComponent<ProjectFunctionsProps> = ({
  functions,
}) => {
  ...
  const filtered = useMemo(() => {
    ...
    return functions.filter((func) => {
      ...
      return (!func.functionType || func.functionType === "user") &&
        !id?.startsWith("pikku");
    });
  }, [functions]);
  ...
  const columns = useMemo(() => [
    {
      ...
      render: (func: any) => {
        ...
        return (
          <>
            <Text fw={500} truncate>{funcId}</Text>
            <Text size="xs" c="dimmed" truncate style={!func.summary && !func.description ? { opacity: 0.4 } : undefined}>{func.summary || func.description || "No description"}</Text>
          </>
        );
      },
    },
    {
      ...
      render: (func: any) => (
        <Group gap={6} wrap="nowrap">
          ...
          {func.expose === true && (
            <PikkuBadge type="flag" flag="exposed" />
          )}
          {func.internal === true && (
    ...
    },
    {
      key: "auth",
      header: "AUTH",
      render: (func: any) => func.sessionless !== true
    ...
    },
    {
      key: "permissions",
      header: "PERMISSIONS",
      render: (func: any) => func.permissions?.length > 0
    ...
    },
    {
      ...
      render: (func: any) => {
        ...
        const usedByCount = usedBy ? usedBy.transports.length + usedBy.jobs.length : 0;
        if (usedByCount === 0) return null;
        return <PikkuBadge type="dynamic" badge="wirings" value={usedByCount} />;
      },
    },
  ], [functionUsedBy]);
  ...
  return (
    <TableListPage
      ...
      onRowClick={(func) => openFunction(func.pikkuFuncName || func.pikkuFuncId, func)}
